...
namespace Zenfolio.Common.Testing.Assertions
{
    public class ValidationResultAssertions : ReferenceTypeAssertions<ValidationResult, ValidationResultAssertions>
    {
        ...
        {
            Ensure.Any.IsNotNull(expression, nameof(expression));
            Ensure.Collection.HasItems(errors, nameof(errors));
        ...
        private static string TransformPropertyName(string error, string propertyName) =>
            error.Replace(CommonConstants.ValidationPropertyName, propertyName, StringComparison.Ordinal);
